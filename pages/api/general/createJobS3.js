...
import {SaveJobStep3ToMoralisDB} from "../../../JS/DB-pushFunctions.js"
...
apiRoute.post(async (req, res) => {
  ...
  const _seller = ValidateAndReturnMessage(address, req.body.message_seller[0].toString(), req.body.signature_seller[0].toString());
  const seller = _seller.toLowerCase();
  ...
  if(AnyEmpty([seller, objectId])){ //title, seller, jobCategory, jobSubCategory, jobDescription, 
  ...
  await SaveJobStep3ToMoralisDB(objectId, seller, imageLinks); //, title, jobCategory, jobSubCategory, jobDescription
