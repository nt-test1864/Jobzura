...
import {SaveJobStep2ToMoralisDB} from "../../../JS/DB-pushFunctions.js"
...
apiRoute.post(async (req, res) => {
  ...
  const jobPriceCurrencyBasic = ValidateAndReturnMessage(address, req.body.message_jobPriceCurrencyBasic[0].toString(), req.body.signature_jobPriceCurrencyBasic[0].toString());
  const jobPriceBasic = ValidateAndReturnMessage(address, req.body.message_jobPriceBasic[0].toString(), req.body.signature_jobPriceBasic[0].toString());
  const jobDeliveryTime = ValidateAndReturnMessage(address, req.body.message_jobDeliveryTime[0].toString(), req.body.signature_jobDeliveryTime[0].toString());
  ...
  if(AnyEmpty([objectId, jobPriceCurrencyBasic, jobPriceBasic, jobDeliveryTime])){ //seller, 
  ...
  console.log(`jobPriceCurrencyBasic: ${jobPriceCurrencyBasic}`)
  console.log(`jobPriceBasic: ${jobPriceBasic}`)
  console.log(`jobDeliveryTime: ${jobDeliveryTime}`)
  ...
  await SaveJobStep2ToMoralisDB(objectId, seller, jobPriceCurrencyBasic, jobPriceBasic, jobDeliveryTime);
